...
*[LP] §9.1 (Networks and Reversible Markov Chains) and §12.2 (Relaxation Time / spectral gap). Lovász, "Random walks on graphs: A survey" (1993) — a readable standalone reference. For the Laplacian connection: Spielman, "Spectral Graph Theory" lecture notes (freely available online).*
...
*[LP] §1.5. [HPC] calls this the "stationary measure" and shows that the stationary measure of a trace kernel is the normalized restriction of the parent's stationary measure.*
...
*Entropy rate is not covered in [LP]. [HPC] uses entropy rate as a signature of binding — bound particles show a lowering of entropy rate within their community.*
...
*[LP] §13.3 Example 13.15 (induced chain — LP's term for the trace chain; stationary distribution is the conditional distribution π_A). [HPC] §3, trace order and block matrix formula.*
...
*Standard linear algebra. See also [LP] §12.4 (Product Chains).*
